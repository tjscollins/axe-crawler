 ...
 */
async function main(domain, opts) {
    ...
    Promise.all([...linkQueue].reduce((links, url) => {
            ...
            return links;
        }, []).slice(0, FIRST_LINKS).map(testPage))
        .then(results => {
 ...
 */
async function buildLinkQueue(domain, pageContent, depth = 5) {
    let allLinks = await queueLinks(domain, pageContent, null);
    ...
    for (let i = 1; i < depth; i++) {
        ...
        let newLinks = await Promise.all([...links].map(async function(url) {
            let newPage = await axios.get(url);
            return queueLinks(domain, newPage, null);
        }));

        let newLinkSet = newLinks
...
function removeMedia(url) {
    return !/(uploads\/\d{4}\/\d{2}\/)/.test(url) &&
        ...
        !/\.(wmv|avi|flv|mov|mkv|mp..?|swf|ra.?|rm|as.|m4[av]|smi.?|doc|docx|ppt|pptx|pps|ppsx|jpg|png|gif|pdf)$/.test(url);
}
...
function buildHTMLReport(reports) {
    ...
    try {
        body += marked('## Summary of Violations: ' +
            Object.entries(reports.violations).reduce((total, [url, violations]) => {
    ...
    try {
        body += marked('## Summary of Passing Tests: ' +
            Object.entries(reports.passes).reduce((total, [url, passes]) => {
