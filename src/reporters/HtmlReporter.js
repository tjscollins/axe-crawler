...
const CSS_STYLES =
  '<style> h1 { text-align: center; } ol > li { padding-bottom: 15px; font-weight: 500 } ul > li { font-weight: 400; font-size: 12px; } ol > li > span { font-weight: 400; } thead th { text-align: center; } tr { border: 1px solid lightsalmon; } tbody th, tbody td { padding: 5px; text-align: left; vertical-align: text-top; font-weight: normal; } table.summary-table { width: 100%; }</style>';
...
function initHTML() {
  ...
  writeToFile(`<!doctype html> <html lang="en"><head><title>${
    titleString
  }</title><meta name="viewport" content="width=device-width, initial-scale=1">${STYLE_SHEETS}${
    CSS_STYLES
  }</head><body><div class="container"><div class="row"><div class="col-xs-12">${marked(`# ${titleString}`)}`);
